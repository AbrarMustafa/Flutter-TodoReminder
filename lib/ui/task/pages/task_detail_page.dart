...
import 'package:todoreminder/ui/widget/builder/task_badge.dart';
import 'package:todoreminder/ui/widget/dialog/confirm_dialog.dart';
...
class TaskDetailPageState extends State<TaskDetailPage> {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return ConfirmDialog(
          ...
          confirmOnTap: () {
            localNotify.removeNotification(int.parse(task.taskId)).then((data){
              vm.removeTask(task);
  ...
  onLaterAction(context, task, TaskVM vm) {
    ...
    if (task.status == TaskStatus.later) {
      ...
      localNotify.showNotificationWithDelay(int.parse(task.taskId),task);
    } else {
      showSnackBar(
        context: context,
